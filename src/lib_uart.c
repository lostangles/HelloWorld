...
void uart_tx_via_dma(void) {
	...
	dma->PAR = &usart->DR;
	dma->M0AR = uart_tx_buffer;
	dma->NDTR = i;
	dma->CR = (0x07 << DMA_SxCR_CHSEL_Pos) | (DMA_SxCR_MINC) | (DMA_SxCR_DIR_0) | (DMA_SxCR_EN);
	//dma->CCR &= !DMA_CCR_EN;                             // disable the channel
	//dma->CNDTR = i;                                      // bytes to transfer
	//dma->CPAR = (uint32_t) &usart->TDR;                  // peripheral address
	//dma->CMAR = (uint32_t) uart_tx_buffer;               // memory address
	//dma->CCR = DMA_CCR_MINC | DMA_CCR_DIR | DMA_CCR_EN;  // increment memory address, read from memory, enable the channel
